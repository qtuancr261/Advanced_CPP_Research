...
void prepareJobs(uint32_t seed) {
    ...
    while (jobGen < MAX_JOBS) {
        ...
        // std::this_thread::sleep_for(std::chrono::milliseconds(10));
        {
            ...
            // any thread in a blocked or waiting state wakes from its slumber
            // (unblocks it) reacquires the lock on mutex and check the condition
            // again
...
void processJobs() {
    while (totalProcessJobs < MAX_JOBS * std::thread::hardware_concurrency() / 2) {
        ...
        jobCondition.wait(lock, /* the condition being waited for */ [] {
            return (((totalProcessJobs >= MAX_JOBS * std::thread::hardware_concurrency() / 2) && (jobsQueue.empty())) || !jobsQueue.empty());
        });
        // condition_variable::wait() check the condition
        // the condition is satisfied => return, still lock the mutex
        // the condition isn't satisfied => unlock the mutex and put the current
        // thread in a blocked or waiting state During a call to wait(), the
        // condition variable may check the supplied contidion any number of times.
        // It always does so with the mutex locked and will return immediately if
        // the function provided to test the condition return true
        if (jobsQueue.empty()) break;
        uint32_t processJob{jobsQueue.front()};
        // std::cout << "\n Thread " << std::this_thread::get_id() << "is processing
        // job: " << processJob << std::endl;
        jobsQueue.pop();
        lock.unlock();
        ++totalProcessJobs;
...
int main() {
    // condition_variable manual test
    ...
    //        worker.join();
    //    }
    // end condition_variable manual test
    // codition_variable wrapper inside ThreadSafeQueue
    ...
    // end codition_variable wrapper inside ThreadSafeQueue
    std::future<unsigned int> seedResult = std::async(genSeed);
    // do somthing else
    auto manualSeed = randSeed();
    std::cout << "seedResult is " << seedResult.get() << std::endl;
    return 0;
}
